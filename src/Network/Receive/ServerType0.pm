...
sub new {
	...
	$self->{packet_list} = {
		...
		'09CB' => ['skill_used_no_damage', 'v2 v a4 a4 C', [qw(skillID lv amount targetID sourceID success)]],
		'0A36' => ['test_packet', 'a4 C', [qw(maybe_ID rest)]],
	};
...
sub test_packet {
	my ($self, $args) = @_;
	my $monster = Actor::get($args->{maybe_ID});
	my $hex = getHex($args->{maybe_ID});
	message("ID $monster->{binType} $monster->{name} restante $args->{rest} ?\n");
}
